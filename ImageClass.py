...
class image:
    ...
    def image_mixer(self, imageToBeMixed, magnitudeOrRealRatio: float, phaesOrImaginaryRatio: float, mode):
        ...
        if mode in self.Modes:
            ...
            if (mode in self.mag_phase_list ):
                combined_image = np.multiply(mode_data1_mixing, np.exp(1j * mode_data2_mixing))
                logger.info("Mixing Magnitude and Phase")
            elif (mode in self.phase_mag_list):
                combined_image = np.multiply(mode_data2_mixing, np.exp(1j * mode_data1_mixing))
            elif  mode =='realAndimaginary':
                combined_image =  mode_data1_mixing + mode_data2_mixing * 1j
                logger.info("Mixing Real And Imaginary")
            elif mode == 'imaginaryAndreal':
                combined_image =  mode_data2_mixing + mode_data1_mixing * 1j
            mixed_image = np.real(np.fft.ifft2( combined_image))  
        return(mixed_image)
